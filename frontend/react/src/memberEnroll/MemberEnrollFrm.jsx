...
//* 닉네임 체크
  const handleNickname=(e)=>{
    ...
    }else if(/([^a-z가-힣0-9\x20\-_])/i.test(nickname)){ // 자음,모음 단독사용시 true 반환.(영문, 일반한글은 false으로 통과)
...
function ToggleAll(props){
  ...
  const toggleAll=(e)=>{
    ...
    const targetCheckboxs = document.querySelectorAll(`[name="${props.target}"]`);
  ...
  return(
    <div>
      <button type='button' id={`${props.target}-toggle`} onClick={toggleAll}>전체 선택</button>
    </div>
  )
}
